...
import { type HDWallet, type TestNetHDWallet, type TestNetWallet, type TokenI, type Utxo, type Wallet } from "mainnet-js";
 ...
 * @param {Wallet | TestNetWallet | HDWallet | TestNetHDWallet } options.wallet - The wallet to use for funding the trade.
...
export const fundProposedTrade = async ({
    ...
    wallet: Wallet | TestNetWallet | HDWallet | TestNetHDWallet,
  ...
  const walletCache = wallet.walletCache;
  const utxoList: Utxo[] = utxos ?? await wallet.getUtxos();
  ...
  const includedUtxoList: Utxo[] = [];
  for (const tokenBalance of tokenBalances) {
    while (tokenBalance.value < 0n) {
      // select from utxo_list
      let subUtxoList
      if (tokenBalance.tokenId === NATIVE_BCH_TOKEN_ID) {
        subUtxoList = utxoList.filter((a) => includedUtxoList.find((b) => a === b) === undefined && a.satoshis > 0n);
        subUtxoList.sort((a, b) => Number(b.satoshis - a.satoshis));
      } else {
        subUtxoList = utxoList.filter((a) => includedUtxoList.find((b) => a === b) === undefined && a.token?.category === tokenBalance.tokenId && a.token?.nft?.capability === undefined && a.token?.nft?.commitment === undefined && a.token.amount > 0n);
        bigIntArraySortPolyfill(subUtxoList, (a: Utxo, b: Utxo) => (b.token as TokenI).amount - (a.token as TokenI).amount);
      }
      ...
      const privateKey = walletCache.get(utxo.address)?.privateKey;
      if (privateKey === undefined) {
        throw new Error(`Private key not found in wallet cache for address ${utxo.address}`);
      }
      const walletLockingBytecode = privateKeyToP2pkhLockingBytecode({ privateKey: privateKey, throwErrors: true })
      inputCoins.push({
  ...
  const privateKey = walletCache.get(depositAddress)?.privateKey;
  if (privateKey === undefined) {
    throw new Error(`Private key not found in wallet cache for address ${depositAddress}`);
  }
  const walletLockingBytecode = privateKeyToP2pkhLockingBytecode({ privateKey: privateKey, throwErrors: true })
 ...
 */
export const broadcastTrade = async (wallet: Wallet | TestNetWallet | HDWallet | TestNetHDWallet, tradeTxList: TradeTxResult[]) => {
  const txIds: string[] = [];
