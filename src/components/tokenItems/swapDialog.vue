<script setup lang="ts">
  ...
  import { type HDWallet, type TestNetHDWallet, type TestNetWallet, type Wallet } from 'mainnet-js';
  ...
  function swapAssets() {
    ...
    void amountAChange({ target: { value: amountBValue } } as unknown as Event);
  }
  ...
  function maxClick() {
    if (assetA.value === NATIVE_BCH_TOKEN_ID) {
      const satBalance = Number(store.balance ?? 0n);
      if (satBalance < 10000) {
        return;
      }
      amountA.value = String((satBalance - 10000) / 1e8);
      void amountAChange({ target: { value: String((satBalance - 10000) / 1e8) } } as unknown as Event);
    } else {
      amountA.value = String(Number(props.tokenBalance) / 10 ** decimalsA.value);
      void amountAChange({ target: { value: String(Number(props.tokenBalance) / 10 ** decimalsA.value) } } as unknown as Event);
    }
  }
  ...
  async function swapClick() {
    ...
    try {
      ...
      const tradeTxList = await fundProposedTrade({wallet: store.wallet as Wallet | TestNetWallet | HDWallet | TestNetHDWallet, tradeProposal: tradeProposal.value});

      await broadcastTrade(store.wallet as Wallet | TestNetWallet | HDWallet | TestNetHDWallet, tradeTxList);
    ...
    void amountAChange({ target: { value: amountA.value } } as unknown as Event);
  }
  ...
  const electrumClient = new ElectrumClient("Cashonize", "0.7.1", webSocket, {
